...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && (int) ($_FILES['image']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
        if ((int) $_FILES['image']['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($extension, $allowedExtensions, true)) {
                ...
                $error = 'Kapak görseli en fazla 5 MB olabilir.';
            } else {
    ...
    if ($title === '' || $isbn === '') {
        ...
        try {
            ...
            $stmt = db()->prepare(
                'INSERT INTO books (title, author_id, category_id, isbn, description, summary, price, image_path, total_copies, available_copies)
                 VALUES (:title, :author_id, :category_id, :isbn, :description, :summary, :price, :image_path, :total_copies, :available_copies)'
            );
            $stmt->execute([
                'title' => $title,
                'author_id' => $authorId,
...
<section class="form-card">
    ...
    <form method="post" action="" enctype="multipart/form-data">
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                <label for="image">Kapak Görseli Seç</label>
                <input class="form-control" id="image" name="image" type="file" accept=".jpg,.jpeg,.png,.webp,.gif,image/*">
                <small class="muted">Telefon veya bilgisayardan görsel seçebilirsin. Maksimum 5 MB.</small>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="summary" name="summary" rows="8" placeholder="Kitabın sağ tarafta gösterilecek özetini yazın..."><?= e($_POST['summary'] ?? '') ?></textarea>
        </div>
